...
import (
	...
	"kitsa.ai/clinical-trial-parser/utils/common/param"
	"kitsa.ai/clinical-trial-parser/utils/common/trie"
	"kitsa.ai/clinical-trial-parser/utils/common/util/text"
...
func Load(s3URL string) (*Variables, error) {
    ...
    for {
        ...
        if err != nil {
            return nil, fmt.Errorf("S3 URL: %s - %v", s3URL, err)
        }
        if len(line) < 8 {
            // Handle cases where there are fewer than 8 columns
            glog.Warningf("S3 URL: %s - line has fewer than 8 columns: %v", s3URL, line)
            continue // Skip lines with fewer columns
        }
    ...
    glog.Infof("Number of variables loaded from S3: %d\n", variables.Size())
