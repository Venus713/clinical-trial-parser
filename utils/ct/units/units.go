...
func Load(s3URL string) (*Units, error) {
    ...
    for {
        ...
        if len(line) < 5 {
            return nil, fmt.Errorf("S3 URL: %s - too few columns, at least 5 needed: %v", s3URL, line)
        }
        id := ID(line[0])
        name := line[1]
        display := line[2]
    ...
    glog.Infof("Number of units loaded from S3: %d\n", units.Size())
